...
async function ensureContractsRootFolder(drive, appFolderId) {
  // FIX 2026-08-20 (live production bug, reported by Anton: "Update Contract"
  // generating nothing even though the master template Doc was confirmed
  // sitting right there in the real, top-level "AA Scooters Contracts"
  // folder). Root cause: TWO folders share the exact name "AA Scooters
  // Contracts" in this Drive -- the real, long-lived top-level one (parented
  // directly under "My Drive", the one Code.gs always used and where every
  // real customer folder + the master template Doc live), and a second,
  // stale one nested under "AA Scooters App Data" (an abandoned artifact of
  // the 2026-08-15 migration attempt -- see this function's own git history;
  // Anton confirmed leaving it in place rather than deleting it was fine).
  // findNamedFolderAnywhere's unscoped, unordered search has no way to
  // prefer one over the other, so which one a given request resolved to was
  // effectively down to whatever order Drive's API happened to return
  // results in that moment -- confirmed live: a passport-photo upload
  // resolved the correct top-level folder while a contract-generate request
  // minutes later resolved the stale nested one instead, producing a
  // "template not found" error even though the template genuinely existed.
  // Same fix as ensureAppFolder's own 'root' in parents scoping above, for
  // the identical class of bug -- try the top-level match FIRST (exact,
  // deterministic), only falling back to the unscoped anywhere-search below
  // for resilience if that somehow comes up empty (e.g. the real folder gets
  // moved out from under "My Drive" some day).
  const escapedRootName = CONTRACTS_FOLDER_NAME.replace(/'/g, "\\'");
  const rootLevelRes = await drive.files.list({
    q: `name = '${escapedRootName}' and mimeType = 'application/vnd.google-apps.folder' and 'root' in parents and trashed = false`,
    fields: 'files(id, name)',
    spaces: 'drive'
  });
  const rootLevelMatch = rootLevelRes.data.files && rootLevelRes.data.files[0];
  if (rootLevelMatch) return rootLevelMatch.id;
...
// ---- Grants "anyone with the link, view only" access to a file, if it
// doesn't already have it. Added 2026-08-20 for Contract PDFs specifically
// (see contract.html's viewContract() and api/contracts/[...path].js's
// 'makeContractPublic' route) -- Anton hit Drive's "you need permission,
// request access" screen viewing a contract from a Chrome profile signed
// into a different Google account than the file owner
// (aascooterchiangmai@gmail.com). Checks existing permissions first so
// repeat calls (every "View Contract" click) don't pile up duplicate
// "anyone" grants.
//
// Extended 2026-08-27 (Anton, live) to Receipt and Checklist PDFs too --
// same permission screen, same fix. Now called from
// generateReceiptDocumentFromJson/generateChecklistDocumentFromJson (in
// lib/contractDocGen.js) at generation time, plus the same
// 'makeContractPublic' backstop route as a retroactive fix for anything
// generated before this existed (see contract.html's
// viewReceipt/viewChecklist). Still DELIBERATELY NOT used for passport
// photos or any other document -- those must stay private. This is scoped
// only by which callers choose to invoke it, not by anything in this
// function itself, so double-check any new caller before reusing this. ----
async function ensureFilePubliclyViewable(drive, fileId) {
  const existing = await drive.permissions.list({ fileId, fields: 'permissions(id, type, role)' });
  const alreadyPublic = (existing.data.permissions || []).some((p) => p.type === 'anyone');
