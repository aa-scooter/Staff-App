...
//
// UPDATE 2026-09-01: "self-heal the moment anything triggers
// recomputeMonthlySummaryCascadeB" turned out to NOT be a safe assumption on
// its own -- confirmed by real September 2026 data sitting genuinely empty
// after creation (the rollover that should have made it had been missed --
// see createCurrentMonthSheetFromJson below) with nothing ever landing a
// write, so Template's own stale literal numbers stayed on cash/bank/bank
// less deposit/total, looking exactly like real (but completely wrong)
// money. Left the exclusion here as-is (still correct: this module has no
// business computing those formula-derived cells itself, and shouldn't
// start duplicating recomputeMonthlySummaryCascadeB's logic here) -- the
// actual fix is api/admin/reset.js's healFreshSheetSummary, which now calls
// that same real cascade proactively, once, right after a sheet actually
// gets created, instead of trusting a future write to get around to it.
...
//
// contractRowCol added 24/08/2026, alongside the matching change to every
// lib/*Writes.js copy of DEPOSIT_CATEGORIES_B (bikesWrites.js,
// customersWrites.js, contractWrites.js) -- a hidden column holding the
// linked Contract row number, so a later feature can find the RIGHT
// deposit to draw down by a stable link instead of matching on customer
// name alone. This carries a still-open deposit's link FORWARD every month
// it survives -- without this, the whole point of the link (surviving a
// customer's rental spanning a month boundary) would silently break the
// first time rollover ran, since copyDepositCategoryRows below only ever
// copies the columns THIS constant lists. Columns 26/27/28 confirmed free
// against real August.json/template.json data (nothing on this sheet has
// ever used column 26 or later). ----
const DEPOSIT_CATEGORIES = [
  { key: 'bank',    dateCol: 15, amountCol: 16, nameCol: 17, contractRowCol: 26 },
  { key: 'wise',    dateCol: 18, amountCol: 19, nameCol: 20, contractRowCol: 27 },
  { key: 'revolut', dateCol: 22, amountCol: 23, nameCol: 24, contractRowCol: 28 }
];
...
// fully-blank row (all 3 -- now 4, see below -- columns empty) is skipped
// rather than copied as a gap, so a real row further down shifts up to
// fill it -- this is the original's own behavior, not something introduced
// here (confirmed against Code.gs's own `continue` on a blank row, then a
// sequential `setValues` write starting at row 2). Scanning stops at the
// first row whose date column reads "total" (case/whitespace-insensitive),
// same as the original. Returns the number of rows copied.
//
// contractRowCol (added 24/08/2026) rides along as a 4th value on the same
// triple/quad -- carried forward exactly like date/amount/name, blank-or-
// not-blank doesn't affect the gap-row emptiness check (a deposit logged
// before this feature shipped has no linked row yet, and that's fine: it
// carries forward as blank, same as it would have carried forward as a
// blank cell before this change existed at all). ----
function copyDepositCategoryRows(sourceRows, newRows, cat) {
  ...
  for (let r = 2; r <= sourceRows.length; r++) {
    ...
    const contractRowRaw = row[cat.contractRowCol - 1];
    if (dateEmpty && amtEmpty && nameEmpty) continue; // gap row -- skip, keep scanning down
    collected.push([dateRaw, amtRaw, nameRaw, contractRowRaw]);
  }
  collected.forEach((quad, i) => {
    const destRow = i + 2;
    writeCell(newRows, destRow, cat.dateCol, quad[0]);
    writeCell(newRows, destRow, cat.amountCol, quad[1]);
    writeCell(newRows, destRow, cat.nameCol, quad[2]);
    writeCell(newRows, destRow, cat.contractRowCol, quad[3]);
  });
...
// ---- Shared core for both "next month" (preemptive rollover) and "this
// month" (recovery -- see createCurrentMonthSheetFromJson below) sheet
// creation. Both are the exact same mechanical operation -- copy Template,
// carry forward figures from the sheet immediately BEFORE the target month
// -- they only differ in which two months (source, target) that is.
// Extracted 2026-09-01 when createCurrentMonthSheetFromJson was added
// (see its own header comment for why), rather than duplicating this
// whole body a second time. ----
async function createMonthSheetFromPreviousCore(sheetIO, params) {
  const { sourceMonthIndex, sourceYear, targetMonthIndex, targetYear, dryRun, sourceDescriptor } = params;
  const sourceMonthName = MONTH_SHEET_NAMES[sourceMonthIndex];
  ...
  const warnings = [];
  const source = await sheetIO.fetchSheetWithMeta(sourceMonthName, sourceYear);
  if (!source.rows || !source.rows.length) {
    warnings.push(
      `Could not find a sheet for ${sourceDescriptor} ("${sourceMonthName} ${sourceYear}") -- ` +
...
async function createNextMonthSheetFromJson(sheetIO, opts) {
  ...
  return createMonthSheetFromPreviousCore(sheetIO, {
    sourceMonthIndex: currentMonthIndex,
    sourceYear: currentYear,
    targetMonthIndex,
    targetYear,
    dryRun,
    sourceDescriptor: 'the current month'
  });
}

// ---- Recovery path for exactly the gap this project hit going into
// September 2026: accounts.html shows "No sheet found matching September"
// because THIS month's own sheet was never created -- the nightly cron (or
// a manual click of the "Create Next Month Sheet" button) needed to run on
// Aug 31 to build it from August, and didn't. createNextMonthSheetFromJson
// above can't fix this by itself once the calendar has already turned over:
// called today it would compute target = October (now + 1), not September,
// and would carry forward from a September sheet that doesn't exist --
// leaving September blank forever and October's opening balances wrong.
//
// This function targets THIS month instead, sourced from the month right
// before it -- the one thing createNextMonthSheetFromJson can't do.
// Deliberately NOT parameterized by an arbitrary target month: it always
// targets now.getMonth()/now.getFullYear(), computed server-side, so there
// is no way for a caller (a page's fetch body, say) to point this at some
// other month by passing one in. accounts.html's own confirm-and-create
// flow (added 2026-09-01, see its offerToCreateCurrentMonthSheet) relies on
// exactly that constraint to safely offer "generate it?" only for the
// month the app is actually looking at right now, never for a month
// someone merely navigated forward to (e.g. browsing ahead to October
// while September is still current -- that must stay a plain error, not an
// offer to create October early). ----
async function createCurrentMonthSheetFromJson(sheetIO, opts) {
  const dryRun = !!(opts && opts.dryRun);
  const now = (opts && opts.now) || new Date(); // overridable for tests only
  ...
  const sourceYear = targetMonthIndex === 0 ? targetYear - 1 : targetYear;
...
module.exports = {
  ...
  createCurrentMonthSheetFromJson,
