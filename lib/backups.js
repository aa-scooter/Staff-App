...
const {
  ensureNamedFolder, ensureYearFolder, listAppFiles, listSubfolders, readJsonFile, writeJsonFile
} = require('./googleDrive');

const BACKUPS_FOLDER_NAME = 'Backups';

// ---- Claim lock for ensureDailyBackup() -- see that function's own
// comment for the race condition this closes (confirmed happening in
// practice, 20/08/2026: two "auto" backups created seconds apart from two
// page loads that both decided a backup was due before either one's had
// finished creating it). ----
const BACKUP_LOCK_FILENAME = 'backupLock.json';
// How long a claim is honored before being treated as abandoned (e.g. the
// claiming request crashed or timed out mid-backup) and up for grabs
// again -- generous enough to comfortably outlast even a slow ~25-file
// backup, short enough that a genuinely stuck lock doesn't block backups
// for the rest of the day.
const LOCK_CLAIM_TIMEOUT_MS = 10 * 60 * 1000;
...
const BACKUP_EXCLUDE_FILENAMES = new Set(['ai_provider.json', 'ai_keys.json', 'calendar_auth.json', BACKUP_LOCK_FILENAME]);
...
// list call.
//
// FIX 2026-08-20 (confirmed live, same day: two "auto" backups landed
// seconds apart): a plain read-then-create here has a real race window --
// creating a backup takes a couple of seconds (lists + copies ~25 files),
// and it's completely normal to load 2+ pages within that window (nav.js
// runs this same check on EVERY page load), so a second page's check can
// easily see "nothing recent yet" before the first page's backup has
// finished and shows up in listBackups().
//
// FIRST ATTEMPT at this fix (superseded, keeping the note since it's a
// real gotcha worth not re-learning): tried claiming BACKUP_LOCK_FILENAME
// via the same optimistic-concurrency primitive (writeJsonFile's
// expectedModifiedTime -> ConflictError) every other write in this app
// already relies on. A standalone test proved this INSUFFICIENT even
// once the lock file already exists: that check is a client-side
// read-then-compare-then-write, and Drive gives no true atomic
// compare-and-swap through this app's write helper -- two genuinely
// concurrent claims can both complete their read+compare before EITHER
// write lands, so both pass and both proceed. Fine for the "rare
// accidental overlap" every other use of that pattern in this app is
// built for; not fine for a lock whose whole job is handling routine
// concurrent hits.
//
// ACTUAL FIX: claim with a random nonce (no compare-check at all -- both
// concurrent writes are ALLOWED to land), wait a beat, then re-read the
// lock FRESH and check whether OUR nonce is the one that's actually
// stored. Only one write can be the file's final content -- whichever
// claim lands last -- so re-reading after a short settle delay
// deterministically finds exactly one winner regardless of how close
// together the two claims were, without needing true atomicity anywhere.
// Proven with a real concurrent (Promise.all) test, not just reasoned
// about -- see test_backups.js. ----
...
async function ensureDailyBackup(drive, appFolderId) {
  ...
  const { data: lockData } = await readJsonFile(drive, appFolderId, BACKUP_LOCK_FILENAME);
  const lockClaimedAt = (lockData && lockData.claimedAt) ? Date.parse(lockData.claimedAt) : 0;
  if (lockClaimedAt && (Date.now() - lockClaimedAt) < LOCK_CLAIM_TIMEOUT_MS) {
    // Someone else (another page load, moments ago) is already creating
    // today's backup right now -- not our job to also do it.
    return { created: false, backup: null, mostRecentAt: mostRecent ? mostRecent.createdAt : null };
  }

  const myClaimId = Date.now().toString(36) + '-' + Math.random().toString(36).slice(2);
  await writeJsonFile(drive, appFolderId, BACKUP_LOCK_FILENAME, { claimedAt: new Date().toISOString(), claimId: myClaimId }, null, false);
  await sleep(LOCK_SETTLE_MS);
  const { data: verifyData } = await readJsonFile(drive, appFolderId, BACKUP_LOCK_FILENAME);
  if (!verifyData || verifyData.claimId !== myClaimId) {
    // Lost the race -- some other request's claim landed after ours.
    return { created: false, backup: null, mostRecentAt: mostRecent ? mostRecent.createdAt : null };
  }
...
// ---- Manual bulk-delete from the Settings backup list (Anton, 20/08/2026:
// a checkbox per backup + a delete-selected action, since the retention
// prune alone still leaves up to 30 days' worth sitting there and he wants
// to clear old test-run backups out himself rather than waiting on the
// clock). Same soft-delete (trashed:true, recoverable from Drive's own
// trash for a while) as pruneOldBackups -- this is a human clicking
// "delete", not data loss, but there's no reason to make it any more
// permanent than the automatic prune already is. Every id independently
// try/catched so one already-gone/bad id doesn't abort the rest of the
// selection. ----
async function deleteBackups(drive, backupIds) {
  const deleted = [];
  const errors = [];
