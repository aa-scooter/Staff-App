...
async function applyDueBackEventPlan(calendar, plan) {
  ...
  if (plan.action === 'delete') {
    if (plan.existingEventId) {
      try {
        ...
        // A 404 means the event's already gone (someone deleted it by hand,
        // or a previous sweep already cleared it) -- genuinely fine, matches
        // Code.gs's own "couldn't confirm, but don't treat a missing event as
        // fatal" posture, so this case stays silent/self-healing.
        //
        // Anything else (expired/revoked calendar auth, wrong calendar,
        // rate limit, network blip, etc.) used to land here and be silently
        // swallowed too -- only a console.warn nobody was watching. THAT'S
        // the exact bug Anton hit on 23/08/2026 ("I returned both today and
        // it didn't update the calendar entry... I have to do it again
        // now"): computeDueBackEventPlan correctly decided action=delete,
        // this call actually failed, and the failure had nowhere to surface
        // -- the return/swap itself still reported success, so nothing ever
        // told Anton the calendar side didn't happen. Re-thrown now (for
        // anything that isn't a plain 404) so syncDueBackEventForCustomerRow's
        // own catch can turn it into a real `error` string its callers
        // already know how to surface as a "Saved, but: ..." warning instead
        // of a Vercel-log-only console.warn (see that function's own
        // 23/08/2026 comment for the full chain). Still never blocks the
        // underlying bike-status write -- that write already happened (or
        // happens right after) regardless of what this throws here.
        const status = err && (err.code || (err.response && err.response.status));
        if (status === 404 || status === '404') {
          console.warn(`[googleCalendarSync] due-back event delete: eventId=${plan.existingEventId} already gone (404) -- treating as already-cleared.`);
        } else {
          console.warn(`[googleCalendarSync] due-back event delete failed for eventId=${plan.existingEventId}:`, err && err.message);
  ...
  if (existing && existingIsAllDay !== plan.wantsAllDay) {
    try { await calendar.events.delete({ calendarId: 'primary', eventId: plan.existingEventId }); } catch (err) { console.warn(`[googleCalendarSync] due-back all-day/timed swap: old event delete failed for eventId=${plan.existingEventId}:`, err && err.message); }
...
async function applyDeliveryEventPlan(calendar, plan) {
  ...
  if (plan.action === 'delete') {
    if (plan.existingEventId) {
      // TEMP DIAGNOSTIC LOGGING (added 18/08/2026) -- see
      // applyDueBackEventPlan's identical comment above for the full "why".
      // This exact catch is the leading suspect for the live "delivery
      // event doesn't get removed on Rent" report: Vercel's logs confirmed
      // syncDeliveryEventForContractRow correctly computed action=delete
      // with a real existingEventId, yet the event stayed on the calendar
      // -- meaning the actual calendar.events.delete() call below is
      // failing, and this bare catch was swallowing the reason completely.
      try { await calendar.events.delete({ calendarId: 'primary', eventId: plan.existingEventId }); } catch (err) { console.warn(`[googleCalendarSync] delivery event delete failed for eventId=${plan.existingEventId}:`, err && err.message); }
    }
    return '';
  }
  const existing = plan.existingEventId ? await getEventSafe(calendar, plan.existingEventId) : null;
