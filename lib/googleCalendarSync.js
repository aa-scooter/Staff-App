  ...
  if (plan.action === 'delete') {
    if (plan.existingEventId) {
      try {
        ...
        // "stale delivery/due-back event not deleted" report is confirmed
        // fixed): this catch used to swallow the error completely -- a
        // computed action=delete with a real existingEventId showed up
        // correctly in syncDeliveryEventForContractRow's own new log line,
        // but the event stayed on the calendar anyway, and there was
        // nowhere for the ACTUAL Calendar API rejection reason to surface
        // from, since this exact catch is where it would have landed.
