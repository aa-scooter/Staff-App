...
async function getEventSafe(calendar, eventId) {
  ...
  try {
    ...
    // Added 26/08/2026 -- see resolveRealEventId's header comment just
    // below for the full "why": a plain get() fails for any event whose
    // stored id is actually a legacy Apps-Script iCalUID (xxx@google.com),
    // not this API's own event id. Before giving up and treating the
    // event as gone (which is what caused today's "Sync Now duplicated
    // everything" incident -- every legacy-id row got a brand new event
    // instead of being matched to its real, already-existing one), try
    // resolving it as an iCalUID once.
    return await resolveRealEventId(calendar, eventId);
  }
}

// ---- Legacy event-ID resolver (added 26/08/2026) -----------------------
// Root cause of three linked symptoms: "extend doesn't move the due-back
// date", "return doesn't remove the calendar entry", and (worst) today's
// "Sync Now duplicated every entry on the calendar" incident. Events
// created by the OLD Apps Script (Code.gs) backend, before this project's
// calendar sync was rewritten against the Calendar REST API (18/08/2026),
// have their id stored in customer.json/Contract.json in Apps Script's
// CalendarEvent.getId() format -- a plain string ending "@google.com",
// which is actually an iCalUID, NOT the REST API's own internal event id.
// events.get()/events.update()/events.delete() all take `eventId` as their
// OWN internal id -- passed an iCalUID-shaped string instead, they fail
// (typically 404). getEventSafe was already treating that as "gone", and
// every delete call's catch was already treating a 404 as "already gone,
// nothing to do" -- both reasonable-looking assumptions that are wrong
// specifically for these legacy rows. Net effect for every row whose id
// predates the rewrite: upsert could never find the real event (so it
// created a SECOND one instead of updating the first), and delete could
// never find the real event either (so returned bikes' entries never
// actually disappeared).
//
// Fix: when a direct lookup/delete by eventId fails, try resolving it as
// an iCalUID instead -- events.list({iCalUID}) DOES accept and correctly
// look up this legacy format -- and use the REAL id it returns from then
// on. Every upsert already rewrites the row's stored calendarEventId to
// whatever id it used going forward, so a given row only ever needs this
// fallback once -- after that it's carrying a normal REST-API id and never
// touches this path again. Self-healing, no backfill/migration needed.
async function resolveRealEventId(calendar, legacyOrRealId) {
  if (!legacyOrRealId) return null;
  try {
    const res = await calendar.events.list({ calendarId: 'primary', iCalUID: legacyOrRealId, maxResults: 1, showDeleted: false });
...
// Deletes an event whether `idOrLegacyUid` is a real REST-API event id or
// a legacy Apps-Script iCalUID (see resolveRealEventId above) -- shared by
// applyDueBackEventPlan and applyDeliveryEventPlan's own 'delete' branches
// so the fix lives in exactly one place. A clean 404 on the direct attempt
// might mean "genuinely already gone" OR "that id was never a real
// eventId to begin with (it's an iCalUID)" -- only the iCalUID resolution
// step below can tell those apart, so it always runs before concluding an
// event is really gone.
async function deleteEventByIdOrICalUid(calendar, idOrLegacyUid) {
  if (!idOrLegacyUid) return;
  try {
    await calendar.events.delete({ calendarId: 'primary', eventId: idOrLegacyUid });
    return;
  } catch (err) {
    const status = err && (err.code || (err.response && err.response.status));
    if (status !== 404 && status !== '404') {
      throw new Error('could not remove the old calendar event (' + ((err && err.message) || 'unknown error') + ')');
    }
  }
  const resolved = await resolveRealEventId(calendar, idOrLegacyUid);
  if (!resolved) return; // genuinely already gone / never existed -- fine
  try {
    await calendar.events.delete({ calendarId: 'primary', eventId: resolved.id });
  } catch (err2) {
    const status2 = err2 && (err2.code || (err2.response && err2.response.status));
    if (status2 === 404 || status2 === '404') return; // gone between the list() and this delete -- fine
    throw new Error('could not remove the old calendar event (' + ((err2 && err2.message) || 'unknown error') + ')');
  }
}
...
async function applyDueBackEventPlan(calendar, plan) {
  ...
  if (plan.action === 'delete') {
    // 26/08/2026: now routed through deleteEventByIdOrICalUid (see its own
    // header comment) instead of a raw calendar.events.delete() try/catch
    // -- a plain delete-by-id silently no-op'd (treated as "already gone")
    // for every legacy Apps-Script-created event, which is exactly why
    // returning a bike was never actually removing its calendar entry.
    await deleteEventByIdOrICalUid(calendar, plan.existingEventId);
    return '';
  }

  const existing = plan.existingEventId ? await getEventSafe(calendar, plan.existingEventId) : null;
  // 26/08/2026: prefer existing.id (the REAL event id, possibly resolved
  // from a legacy iCalUID by getEventSafe/resolveRealEventId) over
  // plan.existingEventId for every further API call below -- using the
  // stored legacy id directly is what made update()/delete() fail and
  // fall through to creating a brand new duplicate event instead.
  const realExistingId = (existing && existing.id) || plan.existingEventId;
  ...
  if (existing && existingSignature === plan.syncSignature) {
    return realExistingId; // nothing changed and the event is still there
  }
  ...
  if (existing && existingIsAllDay !== plan.wantsAllDay) {
    try { await calendar.events.delete({ calendarId: 'primary', eventId: realExistingId }); } catch (err) { console.warn(`[googleCalendarSync] due-back all-day/timed swap: old event delete failed for eventId=${realExistingId}:`, err && err.message); }
    const created = await calendar.events.insert({ calendarId: 'primary', requestBody: resource });
    return created.data.id;
  }

  if (existing) {
    const updated = await calendar.events.update({ calendarId: 'primary', eventId: realExistingId, requestBody: resource });
...
// Returns { row: updatedRowCopy, changed: bool, error?: string } so the
// caller decides whether a sheet write-back is needed. Never throws
// (matches Code.gs's own "a calendar problem should never break the row
// write it's piggybacking on" posture) -- logs and returns the row
// unchanged on failure.
//
// `error` added 23/08/2026 (Anton: "I returned both today and it didn't
// update the calendar entry... I have to do it again now" -- the 22/08
// return/swap fix wired this function in correctly, but THIS function's
// own catch was swallowing the actual Calendar API failure reason
// completely -- only a console.warn nobody was watching, the same
// silent-failure shape this file's own TEMP DIAGNOSTIC comments already
// document for the delivery-event bug. Every caller in bikesWrites.js
// already has a `warnings`-array + frontend `alert('...but: ' +
// result.warning)` pattern for exactly this kind of best-effort
// follow-up failure -- this just gives them the message to put in it, so
// a real failure is visible in the UI on the next attempt instead of
// only in Vercel's logs. ----
async function syncDueBackEventForCustomerRow(calendar, row, contractLookup) {
  ...
  try {
    ...
    const message = (err && err.message) || 'unknown calendar sync error';
    console.warn('[googleCalendarSync] due-back sync failed:', message);
    return { row, changed: false, error: message };
  }
}
...
async function applyDeliveryEventPlan(calendar, plan) {
  if (plan.action === 'skip') return plan.existingEventId || '';
  if (plan.action === 'delete') {
    // 26/08/2026: see applyDueBackEventPlan's identical change and
    // deleteEventByIdOrICalUid's own header comment -- same legacy-id fix,
    // same "delete on Rent doesn't actually remove the old delivery event"
    // symptom this file's 18/08/2026 TEMP DIAGNOSTIC comment already
    // suspected, now root-caused.
    await deleteEventByIdOrICalUid(calendar, plan.existingEventId);
    return '';
  }
  const existing = plan.existingEventId ? await getEventSafe(calendar, plan.existingEventId) : null;
  ...
  const existingSignature = existing && existing.extendedProperties && existing.extendedProperties.private
    ? existing.extendedProperties.private.syncSignature : null;
  if (existing && existingSignature === plan.syncSignature) return realExistingId;
...
//
// TEMP DIAGNOSTIC LOGGING (added 18/08/2026, remove once the
// "delivery event doesn't get deleted on Rent" report is confirmed
// fixed): Anton reported that renting a Pending+delivery contract left
// the old 🏨 delivery event on the calendar instead of removing it, even
// though computeDeliveryEventPlan's own `!isPending -> delete` branch
// looks correct on paper. Logging the plan's action + existingEventId
// here (not inside computeDeliveryEventPlan itself, which stays pure/
// network-free on purpose) makes it possible to tell apart the two real
// candidate causes from Vercel's logs on the next test: (a) this function
// never even got called with the right row (e.g.
// markMatchingContractAsRentedFromJson's name/bike match failed to find
// the Pending row at all -- see that function's own new log in
// contractWrites.js), vs (b) it WAS called, but computed something other
// than 'delete' (e.g. still saw status as 'pending', or had no
// existingEventId to delete in the first place).
async function syncDeliveryEventForContractRow(calendar, row) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      console.log(`[googleCalendarSync] delivery plan for "${(row[CT.NAME] || '').toString().trim()}" / "${(row[CT.BIKE] || '').toString().trim()}": action=${plan.action}, existingEventId=${plan.existingEventId || '(none)'}, status="${(row[CT.STATUS] || '').toString().trim()}"`);
      if (plan.action === 'skip') return { row, changed: false };
      ...
      lastErrorMessage = (err && err.message) || 'unknown calendar sync error';
      console.warn(`[googleCalendarSync] delivery sync attempt ${attempt + 1} failed:`, lastErrorMessage);
    }
  }
  // Added 25/08/2026 (see dailySweep's error-collection addition just below,
  // and api/contract/write.js's calendar_sweep_log.json write) -- both
  // attempts failed; surface why instead of returning changed:false
  // indistinguishable from "nothing needed to happen".
  return { row, changed: false, error: lastErrorMessage };
}
...
async function createContactCustomerReminders(calendar, customerRows, contractLookup, now) {
  ...
  for (const c of candidates) {
    if (c.staleReminderId) {
      ...
      if (stale) { try { await calendar.events.delete({ calendarId: 'primary', eventId: c.staleReminderId }); } catch (err) { console.warn(`[googleCalendarSync] stale contact-reminder delete failed for eventId=${c.staleReminderId}:`, err && err.message); } }
    }
...
async function dailySweep(calendar, customerRows, contractRows) {
  ...
  // errors[] added 25/08/2026 -- Anton asked for a durable, Claude-readable
  // record of what the nightly sweep actually did/found, since he's away
  // when it runs at 22:00 Bangkok (see api/contract/write.js's
  // handleDailySweepCron, which persists this whole result to
  // calendar_sweep_log.json in the app's Drive folder). Before this, a
  // per-row sync failure was only ever a console.warn inside
  // syncDueBackEventForCustomerRow/syncDeliveryEventForContractRow --
  // invisible outside Vercel's own function logs. Collecting the `error`
  // those two already return (due-back) or now return (delivery, this same
  // commit) is what makes them loggable at all.
  const errors = [];
  ...
  for (let i = 0; i < updatedCustomerRows.length; i++) {
    ...
    const { row, changed, error } = await syncDueBackEventForCustomerRow(calendar, updatedCustomerRows[i], contractLookup);
    if (changed) { updatedCustomerRows[i] = row; dueBackChanged++; }
    if (error) {
      errors.push({
        type: 'dueBack',
        name: (updatedCustomerRows[i][CUST.NAME] || '').toString().trim(),
        bike: (updatedCustomerRows[i][CUST.BIKE] || '').toString().trim(),
        message: error
      });
    }
  }
  ...
  for (let i = 0; i < updatedContractRows.length; i++) {
    ...
    const { row, changed, error } = await syncDeliveryEventForContractRow(calendar, updatedContractRows[i]);
    if (changed) { updatedContractRows[i] = row; deliveryChanged++; }
    if (error) {
      errors.push({
        type: 'delivery',
        name: (updatedContractRows[i][CT.NAME] || '').toString().trim(),
        bike: (updatedContractRows[i][CT.BIKE] || '').toString().trim(),
        message: error
      });
    }
  }
