...
const {
  ...
  readJsonFile, listAllFilesInFolder, trashFile, docsClientFromSession
} = require('./googleDrive');
...
async function generateContractDocumentFromJson(ctx, data) {
  ...
  const bikeModel = stripBikeNameBrackets(data.bikeModel);
  const bikeTaxRows = await getBikeTaxCategories(drive, effectiveFolderId, session);
  const bikeRow = bikeTaxRows.find((r) => bikeNamesMatchForTaxLookup(r.bike, bikeModel));
  const keyType = getKeyTypeForBike(bikeTaxRows, bikeModel);
  const nextOil = await getNextOilChangeForBike(drive, effectiveFolderId, session, bikeModel);
  const cc = bikeRow ? bikeRow.cc : '';
  const plate = bikeRow ? bikeRow.plate : '';
  const bikeDisplayName = buildBikeDisplayName(bikeRow && bikeRow.make, bikeRow && bikeRow.model, bikeModel);
